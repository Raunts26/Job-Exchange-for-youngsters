  ...
  class Misc {
    ...
    function getCategories() {
      ...
      $stmt = $this->connection->prepare("SELECT news_categories.id, name, count(news.id) FROM news_categories
                                          INNER JOIN news ON news.category = news_categories.id WHERE news.deleted IS NULL
    ...
    function addNews($sub, $cat, $text) {
      $response = new StdClass();

      $stmt = $this->connection->prepare("INSERT INTO news (subject, category, text, posted) VALUES (?, ?, ?, NOW())");
      $stmt->bind_param("sis", $sub, $cat, $text);
    ...
    function countNews() {
      ...
      $stmt = $this->connection->prepare("SELECT id FROM news WHERE deleted IS NULL");
      $stmt->bind_result($id);
      $stmt->execute();
