<?php
	//Lehe nimi
	$page_title = "Muuda töövarje";
	//Faili nimi
	$page_file = "editshadows.php";
?>
<?php
...
?>
<?php
	$job_array = $Shadow->getAdminData();

	if(isset($_SESSION['logged_in_user_id'])) {
		if($_SESSION['logged_in_user_group'] == 3) {
			if(isset($_GET["delete"])) {
				$Shadow->deleteJobData($_GET["delete"]);
			}

			if(isset($_GET["update"])) {
				$Shadow->updateJobData($_GET["job_id"], $_GET["job_name"], $_GET["job_email"], $_GET["job_number"], $_GET["job_company"], $_GET["job_desc"], $_GET["job_county"], $_GET["job_parish"], $_GET["job_location"], $_GET["job_address"]);
				header("Location: editshadows.php");
				
			}

			if(isset($_GET["activate"])) {
				$Shadow->activateData($_GET["activate"]);
			}

			if(isset($_GET["deactivate"])) {
				$Shadow->deactivateData($_GET["deactivate"]);
			}
...
<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
	<?php
		for($i = 0; $i < count($job_array); $i++) {
			...
			echo '</div>';
			if(isset($_GET["edit"]) && $_GET["edit"] == $job_array[$i]->id) {
				echo '<form action="editshadows.php" method="get">';
				...
				echo '<input class="form-control" type="text" name="job_company" value="'.$job_array[$i]->company.'">';
				echo '</div>';
				echo '</div>';
				...
				echo '<input class="form-control" type="text" name="job_email" value="'.$job_array[$i]->email.'">';
				echo '</div>';
				echo '</div>';
				...
				echo '<input class="form-control" type="text" name="job_number" value="'.$job_array[$i]->number.'">';
				echo '</div>';
				echo '</div>';
				...
				echo '<input class="form-control" type="text" name="job_county" value="'.$job_array[$i]->county.'">';
				echo '</div>';

				echo '<div class="form-group">';
				echo '<label> Vald </label>';
				echo '<input class="form-control" type="text" name="job_parish" value="'.$job_array[$i]->parish.'">';
				echo '</div>';
				...
				echo '<input class="form-control" type="text" name="job_location" value="'.$job_array[$i]->location.'">';
				echo '</div>';
				...
				echo '<a href="editshadows.php" class="btn btn-danger"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>';
				echo '</div>';
				echo '</div>';
				echo '</div>';
				echo '</div>';
