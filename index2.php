...
<div class="row">
	<div id="welcome_pic" class="parallax-window" data-parallax="scroll" data-image-src="<?$_SERVER['BASE_PATH']?>/ntb/images/parallax/index.jpeg"></div>

	<div class="col-sm-offset-3 col-sm-6" style="padding: 250px 0;">

		<form action="<?$_SERVER['BASE_PATH']?>/ntb/content/jobs.php" method="get">
			<div class="input-group">
				<input name="keyword" type="text" class="form-control index-search input-lg" placeholder="Otsi tööpakkumisi" autocomplete="off">
				<span class="input-group-btn">
					<button class="btn btn-default index-search-btn btn-lg" style="z-index: 1;" type="submit" value="otsi">Otsi</button>
				</span>
			</div>
		</form>
	...
	<div class="col-sm-offset-2 col-sm-8 index-last">
		<h2 class="index-h2">VÄRSKED PAKKUMISED</h2>
	</div>
	...
	<div class="col-sm-offset-2 col-sm-4">
		<div class="list-group">
			...
			<?php for($i = 0; $i < 6; $i++):
							if($job_array[$i]->id === NULL) {
								break;
							}
							if($i % 2 === 0):
			?>

			<a class="list-group-item index-offers" data-toggle="modal" data-target="#<?=$job_array[$i]->id;?>">
				<div class="col-xs-8">
					<h4 class="list-group-item-heading"><?=$job_array[$i]->name;?></h4>
				...
				<div class="col-xs-4 text-right">
					<img class="index-brand" src="<?=$_SERVER['BASE_PATH']?>/ntb/images/logo.png" alt="logo">
				</div>
			</a>
...
<!--<div class="row">
		...
		<div class="list-group">
			<?php
				if (count($job_array) >= 6) {
					for($i = 0; $i < 6; $i++) {
						echo '<a style="cursor:pointer; height: 68.5px;" class="list-group-item" data-toggle="modal" data-target="#'.$job_array[$i]->id.'">';
	...
	<div class="col-xs-12 col-sm-4">
		...
		<div id="news" class="list-group" style="max-height: 408px; overflow: auto;">
		<?php
		if (count($news) >= 3) {
					for($i = 0; $i < 3; $i++) {
						echo '<a href="content/news.php?id='.$news[$i]->id.'" class="list-group-item">';
						echo '<h4 class="list-group-item-heading">'.$news[$i]->subject.'</h4>';

						if (strlen($news[$i]->text) > 350) {
							$str = $news[$i]->text;
							$str = explode( "\n", wordwrap( $news[$i]->text, 350));
							$str = $str[0];
							echo '<p class="list-group-item-text">'.$str.'</p>';
						} else {
	            echo '<p class="list-group-item-text">'.$news[$i]->text.'</p>';
	          }
						echo '</a>';
	...
	<div class="col-xs-12 col-sm-4">
		...
		<div class="list-group">
			<div class="list-group-item" style="height: 408px">
